...
void DrawDetectorDD4hep::drawDD4hepDetector( DD4hep::Geometry::LCDD& lcdd ){
  ...
  //vector of CEDGeoTube - polyhedra drawn in CED 
  std::vector<CEDGeoTube> gTV ;

  int detLayer = NUMBER_DATA_LAYER; //  first non data layer in CED

  //--- draw the calorimeters 
  for( unsigned i=0,n=calorimeters.size() ; i<n ; ++i ){

    DetElement det = calorimeters[i] ;

    std::string detName = det.name() ;

    streamlog_out( MESSAGE ) << " drawing calorimeter: " << detName << std::endl;  
    
    
    LayeredCalorimeterData* calo = 0 ;

    // try to get the LayeredCalorimeterData extension for this calorimeter:
    try{ 
      calo = det.extension<LayeredCalorimeterData>() ; 
      
    } catch(std::runtime_error& e){

      streamlog_out( MESSAGE ) << " calorimeter: " << detName 
    ...
    if( calo != 0 ) {
      ...
      //      if( detName.find("Barrel") != std::string::npos ){
      
      if( calo->layoutType == LayeredCalorimeterData::BarrelLayout ) {
	...
	//---------------------------------------------------------------------------
	// to do: get inner and outer symmetries, phi offset, color from calo or det ... 
	//---------------------------------------------------------------------------

	int layer =  detLayer++ ;

	gTV.push_back( CEDGeoTube( rMax, rMin, 12,  12, 0, 0, zMax,  -zMax, 0xffffff , layer  ,1,1 ) ) ; 
  ...
  for( unsigned i=0,n=trackers.size() ; i<n ; ++i ){
    ...
    std::string detName = trackers[i].name() ;
    streamlog_out( MESSAGE ) << " drawing tracker: " <<  detName << std::endl; 

    //ZPlanarData* trk = det.extension<ZPlanarData>() ;
  ...
  for( unsigned i=0,n=passiveDets.size() ; i<n ; ++i ){
    ...
    streamlog_out( MESSAGE ) << " drawing passive det: " << passiveDets[i].name() << std::endl;  
  }
...
// void DrawDetectorDD4hep::drawDetectorFromGear( gear::GearMgr* gearMgr ){
//     //
